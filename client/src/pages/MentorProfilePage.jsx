...
async function fetchOpenRequest(mentorUserId, menteeId) {
...
export default function MentorProfilePage() {
  ...
  // Auth seam — same source the scheduling personal area reads, so a request
  // created here belongs to the mentee whose Personal Area shows it next.
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      ...
      try {
        ...
        try {
          const open = await fetchOpenRequest(mentorData.userId, menteeId);
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      // Mentee withdraws her own still-open request. Routes through the
      // scheduling state machine (schedulingService.withdraw): legal only from
      // WAITING_FOR_MENTOR_SLOTS / WAITING_FOR_MENTEE_SELECTION, 403 if the
      // actor is not this request's mentee, 409 on any other state. A meeting
      // that is already MATCHED is handled from the Personal Area, not here.
      await axios.post(`/api/requests/${openRequest.id}/withdraw`, {
        actingUserId: menteeId,
      });
      setOpenRequest(null);
      ...
      if (err.response?.status === 409 && mentor?.userId) {
        try {
          const fresh = await fetchOpenRequest(mentor.userId, menteeId);
